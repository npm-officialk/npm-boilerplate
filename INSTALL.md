...
```
__RUNNER__ run start:server -p <port> <folderPath>
```
...
```
__RUNNER__ run fix:fortify
```

**fix vunerability of dependency**

runs `__RUNNER__ audit` to try and fix any package dependency vunerabilities

```
__RUNNER__ run fix:audit
```
...
```
__RUNNER__ run check:vunerability
```
...
```
__RUNNER__ run check:license
```

**check dependency vunerability**

run `__RUNNER__ audit` on the dependencies to check for dependecy vunerabilities

_also run `snyk monitor` in postscript to report to snyk_

```
__RUNNER__ run check:audit
```
